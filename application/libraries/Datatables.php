...
class Datatables {
	...
	private function get_filtering() {
		...
		$cWhere = '';
		for ($i = 0; $i < count($mColArray); $i++)
			if (trim($mColArray[$i]['search']['value']) !== '')
				if ($this->check_cType())
					$cWhere .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $this->ci->db->escape_like_str(trim($mColArray[$i]['search']['value'])) . "%' AND ";
				else
					$cWhere .= $this->select[$this->columns[$i]] . " LIKE '%" . $this->ci->db->escape_like_str(trim($mColArray[$i]['search']['value'])) . "%' AND ";

		if ($cWhere !== '') {
			$cWhere = substr_replace($cWhere, '', -4);
			if ($sWhere !== '') {
				$sWhere = '(' . $sWhere . ') AND (' . $cWhere . ')';
			} else {
				$sWhere = $cWhere;
			}
		}
